...
impl VertexDeclBuilder {
    pub fn add(&mut self, attrib: Attrib, count: u8, kind: AttribType, normalized: Option<bool>, as_int: Option<bool>) -> &mut Self {
        unsafe {
    ...
    pub fn skip(&mut self, count: u8) -> &mut Self {
        unsafe { bgfx_sys::bgfx_vertex_decl_skip(&mut self.decl, count); }

        self
    }

    pub fn end(&mut self) -> VertexDecl {
        unsafe { bgfx_sys::bgfx_vertex_decl_end(&mut self.decl); }
...
impl<'a> VertexBuffer<'a> {
    pub fn new<'b, T>(_context: &'a super::MainContext, verts: &'b [T], decl: &'b VertexDecl, flags: super::BufferFlags) -> VertexBuffer<'a> {
        unsafe {
            let mem_ref = bgfx_sys::bgfx_make_ref(
                verts.as_ptr() as *const libc::c_void,
...
impl<'a> IndexBuffer<'a> {
    pub fn new<'b, T>(_context: &'a super::MainContext, indices: &'b [T], flags: super::BufferFlags) -> IndexBuffer<'a> {
        unsafe {
            let mem_ref = bgfx_sys::bgfx_make_ref(
