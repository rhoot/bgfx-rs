...
fn main() {
	...
	// let mut example = common::make_platform(width, height);

	// let bgfx = bgfx::init(&mut example.platform, None, None, None);
	// bgfx.reset(width, height, reset);
 ...
 //    while !example.process_events(&bgfx, width, height, reset) {
 //        // Set view 0 default viewport.
 //        bgfx.set_view_rect(0, 0, 0, width, height);
